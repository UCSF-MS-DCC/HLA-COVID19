class HomeController < ApplicationController
    ...
    def age_data
        results = ActiveRecord::Base.connection.execute("select age, count(*) from subjects s join projects p on s.project_id = p.id where p.id < 2 and p.id > 10 group by s.age")
        respond_to do |format|
            format.html
    ...
    def sex_data
        results = ActiveRecord::Base.connection.execute("select sex, count(*) from subjects s join projects p on s.project_id = p.id where p.id < 2 and p.id > 10 group by s.sex")
        respond_to do |format|
            format.html
            format.json { render json: { :data => results }, status: :ok }
        end
    ...
    def ethnicity_data
        results = ActiveRecord::Base.connection.execute("select ethnicity, count(*) from subjects s join projects p on s.project_id = p.id where p.id < 2 and p.id > 10 group by s.ethnicity")
        respond_to do |format|
            format.html
            format.json { render json: { :data => results }, status: :ok }
        end
